...
def _extract_text_from_json_value(value: Any, output: list[str]) -> None:
    if isinstance(value, dict):
        for key in ["text", "value", "label", "recognized_text", "content"]:
            maybe = value.get(key)
            if isinstance(maybe, str) and maybe.strip():
                output.append(maybe.strip())
        for nested in value.values():
            _extract_text_from_json_value(nested, output)
        return

    if isinstance(value, list):
        for item in value:
            _extract_text_from_json_value(item, output)
        return
...
def convert_surya_output_to_text(source_file: Path, output_txt: Path) -> int:
    ...
    if suffix == ".json":
        ...
        chunks: list[str] = []
        _extract_text_from_json_value(payload, chunks)
        text = "\n".join(chunks)
        output_txt.write_text(text, encoding="utf-8")
        return len(text)
