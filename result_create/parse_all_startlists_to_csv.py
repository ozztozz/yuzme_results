...
def parse_result_pdf_scores(
    ...
    dedupe_candidates: set[tuple[str, str, str, str | None]] = set()
    ...
    for record in [*records, *special_records]:
        ...
        candidate_key = (name_norm, club_norm, result_text, rank_text)
        if candidate_key in dedupe_candidates:
            continue

        dedupe_candidates.add(candidate_key)
        candidates.append(
            ResultScoreCandidate(
                name_norm=name_norm,
                club_norm=club_norm,
...
def _gemini_rows_from_csv_text(csv_text: str) -> list[dict[str, Any]]:
    ...
    # Expected columns include swimmer_name/club/time/rank. Support both ';' and ','.
...
def parse_result_pdf_scores_gemini(
    result_pdf_path: Path,
) -> tuple[dict[tuple[str, str], str], dict[tuple[str, str], str], list[ResultScoreCandidate]]:
    # If gemini parser is unavailable, fallback to splash parsing path.
    try:
        module = importlib.import_module("gemini_pdf_parser")
    except Exception:
        return parse_result_pdf_scores(
            result_pdf_path=result_pdf_path,
            temp_text_dir=Path("results/_tmp_result_text"),
    ...
    parser_fn = getattr(module, "parse_pdf_to_csv_text", None)
    ...
    csv_text = parser_fn(str(result_pdf_path))
    parsed_rows = _gemini_rows_from_csv_text(str(csv_text or ""))
    ...
    for row in parsed_rows:
        swimmer_name = row.get("swimmer_name") or row.get("name")
        club = row.get("club")
        result_value = row.get("time") or row.get("result")
        ...
        rank_text = str(rank_value).strip() if rank_value not in (None, "") else None
...
def parse_startlist_pdf(
    ...
    event_order_raw = extract_list_number(pdf_path)
    event_order = int(event_order_raw) if event_order_raw and event_order_raw.isdigit() else 0
